...
public:
/*設置攝像頭參數計算,*/
/*ir_to_color_baseline:0.0257    color_to_ir_baseline:-0.0257073
color.fx 619.695     ir.fx475.045      ir.fy475.045
*/
const float ir_to_color_baselineX=25.7;//mm
const float ir_to_color_baselineY=-0.733261 ;//mm          
const float ir_to_color_baselineZ=3.95646;//mm
const float  H=250.0f+4.175f;//400.00f;//攝像頭距離地點的高度 
//const float  T=25.7;//mm   70.068;//78.18f;//兩個攝像頭之間的距離寬度
const float  f=619.695;//mm   589.201294;//456.35427;//595.0f;//攝像頭的焦距  img_color 
const float  f_ir=475.045;//						  img_ir
const float  imgWeidth=640.0;
const float  imgHeigth=480.0;
const float T=ir_to_color_baselineX+(ir_to_color_baselineZ*(320.0-xl)/f);
vector<float> z;
vector<float> x;
vector<float> y;
       ...
       for(int i=0;i<size;i++)
         {
               ...
               //cout<<"xl="<<xl<<"  ";
                float xR=matches[i].first.x;
                float xr=xR-(640.0/2-xR)*(f-f_ir)/f_ir;//
               //cout<<"xr="<<xr<<"  ";
               sum+=abs(f*T/(xl-xr));
