  ...
  /*according to the head yaw anf head pitch,correct the angle*/
  	...
  	if(cm730.Connect() == true)
	{
	  ...
	  while (1&&cv::waitKey(30) !=27) 
	    {//顯示圖像並且將圓形匹配出來
		    dev->wait_for_frames();//Blocks until new frames  are aviaiavle
      ...
      if(currentTimestamp[1]!=lastTimestamp[1])
{
	...
	{//
 		 ...
 		 double angle =mWalk.JointValue[20];
                 mWalk.JointValue[20]+=500.0;
		 //while(angle+2000.0!=mWalk.JointValue[20]){sleep(1);}
  		 sleep(1);
		 /*after the angle is added 2000,execute- the next step*/
                 dev->wait_for_frames();//waitting for the new frame 
      ...
      if(currentTimestamp[1]!=lastTimestamp[1])
{
       lastTimestamp[1] = currentTimestamp[1];
        ...
        if(mWalk.JointValue[19]>=4096.0-200.0&&size!=4)
         {  mWalk.JointValue[19]=2048.0;sleep(1);}
        else  if(size==4)
           {//body anf head 
	    ...
	    sleep(1);       
           ...
           {//body anf head 
            ...
            double angle[4] = {0.0, angleStep , angleStep , angleStep };
            mWalk.start(0.00,4,angle); //walk     
	    while(mWalk.RobotState!=mWalk.Walk_stand)
      		{sleep(1);}    
