...
void timer_handler(const boost::system::error_code& error){
	...
	std::cout << "run time: " << (std::chrono::duration_cast<std::chrono::milliseconds>(diff)).count() << std::endl;
    }
    lastTime_ = hrc::now();*/

    timer_.expires_from_now(std::chrono::nanoseconds(20000000));
    timer_.async_wait(boost::bind(timer_handler, boost::asio::placeholders::error));
    gettimeofday(&LIPM_start,NULL);
    if(LIPM_start.tv_usec-start_time > 23000)
        printf("timing  Severe Error !!!!!!     %.2lf\n",LIPM_start.tv_usec-start_time);
    start_time = LIPM_start.tv_usec;
    mWalk.run();
...
void Parainit()
{
	...
	id = 11;
	mJOINT.SetPGain(id,100);
	mJOINT.SetIGain(id,0);
	mJOINT.SetDGain(id,0);
